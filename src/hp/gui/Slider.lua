...
-- direction
M.DIRECTION_HORIZONTAL  = 1
M.DIRECTION_VERTICAL    = 2
...
function M:initInternal()
    ...
    self._accuracy = 0.1
    self._direction = M.DIRECTION_HORIZONTAL
end
...
function M:createChildren()
    ...
    self._thumb_height = self._thumb:getHeight()
end
...
function M:_valueToLoc(value)
    ...
    local x, y
    if self._direction == M.DIRECTION_HORIZONTAL then
        x = self._thumb_width * 0.5 + (self:getWidth() - self._thumb_width) * v
        y = self._background:getHeight() * 0.5
    else
        x = self._background:getWidth() * 0.5
        y = self:getHeight() - self._thumb_height * 0.5 - (self:getHeight() - self._thumb_height) * v
--         y = self._thumb_height * 0.5 + (self:getHeight() - self._thumb_height) * v
--         y = self:getHeight() - y
    end
    return x, y
end

function M:_locToValue(x, y)
    if self._direction == M.DIRECTION_HORIZONTAL then
        -- normalized
...
function M:_updateDrawables()
    ...
    local x, y = self:_valueToLoc(self._value)
    self._thumb:setLoc(x, y)
    if self._direction == M.DIRECTION_HORIZONTAL then
        self._progress:setPos(0, 0)
        self._progress:setSize(x + self._thumb_width * 0.5, self:getHeight())
    else
        self._progress:setPos(0, y)
        self._progress:setSize(self:getWidth(), self:getHeight() - y)
    end
end
...
-- @return direction
--------------------------------------------------------------------------------
function M:getShowProgress()
    return self._progress:getParent() ~= nil
...
-- @param direction direction
--------------------------------------------------------------------------------
function M:setShowProgress(show)
    if show and not self:getShowProgress() then
        self:removeChild(self._thumb)
        self:addChild(self._progress)
        self:addChild(self._thumb)
    elseif not show and self:getShowProgress() then
        self:removeChild(self._progress)
    end
...
function M:touchMoveHandler(e)
    ...
    if self._touching then
        ...
        local v = self:_locToValue(mx, my)
        self:setValue(v)
    end
end
